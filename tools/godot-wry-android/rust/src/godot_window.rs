...
// Android 上 wry 的后端不使用窗口句柄（WebView 直接叠加到 Activity 内容视图之上），
// 这里提供一个满足 HasWindowHandle 约束的占位实现即可。
...
impl HasWindowHandle for GodotWindow {
    ...
    #[cfg(target_os = "android")]
    fn window_handle(&self) -> Result<WindowHandle<'_>, HandleError> {
        // 句柄不会被解引用（wry Android 后端忽略 window 参数），使用悬垂指针占位。
        let handle = AndroidNdkWindowHandle::new(NonNull::<c_void>::dangling());
        unsafe {
            Ok(WindowHandle::borrow_raw(
