...
/// Android 平台支持：注册 wry 所需的 JNI 回调，并提供给 Kotlin 侧（WryActivity）的初始化入口。
///
/// 注意：包名 `com.example.interactivetext` 同时硬编码在 JNI 符号中，
/// 修改包名时必须同步修改本模块与 Kotlin 源码包名后重新编译。
#[cfg(target_os = "android")]
pub mod android_glue {
    use std::sync::atomic::{AtomicBool, Ordering};
    use wry::prelude::*;
    use wry::prelude::jni::objects::{JClass, JObject};
    use wry::prelude::jni::JNIEnv;
    use wry::prelude::ndk::looper::ThreadLooper;

    // 生成 wry Android 后端所需的全部 JNI 导出符号
    // （RustWebView / RustWebViewClient / Ipc / RustWebChromeClient / WryActivity 的 native 方法）。
    // android_binding! 展开为块表达式，需要放在 const 块中以容纳其中的 fn 项。
    const _: () = {
        wry::android_binding!(com_example, interactivetext);
    };

    /// 应用包名（JNI 路径形式），由 wry 用于查找 Kotlin 粘合类。
    pub const PACKAGE: &str = generate_package_name!(com_example, interactivetext);
    ...
    /// 由 WryActivity（Kotlin 侧）在 Android 主线程的 onCreate 中调用。
    /// 必须在带 Looper 的线程（即 UI 主线程）执行。
    #[no_mangle]
    pub unsafe extern "C" fn Java_com_example_interactivetext_WryActivity_nativeWrySetup<'local>(
        env: JNIEnv<'local>,
        _class: JClass<'local>,
        ...
        let activity = env
            ...
            .expect("[Godot WRY] 无法创建 Activity 的全局引用");
        let looper = ThreadLooper::for_thread()
            .expect("[Godot WRY] nativeWrySetup 必须在 Android 主线程调用");
...
impl WebView {
    ...
    fn build_webview(&mut self) {
        ...
        #[cfg(target_os = "android")]
        if !crate::android_glue::is_setup_done() {
            godot_error!("[Godot WRY] Android 端 wry 尚未初始化：请确认使用自定义 Gradle 构建导出，\
                并将主 Activity 设为 com.example.interactivetext.WryActivity 的子类\
                （见 tools/godot-wry-android/README.md）。");
            return;
        }
