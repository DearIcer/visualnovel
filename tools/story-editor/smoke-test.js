...
;(function runTests() {
  ...
  // 1. 加载项目自带的 main.json（期望值从文件本身读取，适配不同分支的剧本）
  const main = JSON.parse(MAIN_JSON);
  const expectedScenes = Object.keys(main.scenes);
  const firstCharId = Object.keys(main.characters)[0];
  loadDoc(main, 'main.json');
  assert(sceneOrder.length === expectedScenes.length, '应解析出 ' + expectedScenes.length + ' 个场景, 实际 ' + sceneOrder.length);
  assert(currentScene === expectedScenes[0], '当前场景应为 ' + expectedScenes[0]);
  assert(validate(false) === true, '自带 main.json 应通过校验, 问题: ' + JSON.stringify(lastIssues));

  // 2. 序列化应保持场景顺序与结构
  const out = JSON.parse(serializeDoc());
  assert(JSON.stringify(Object.keys(out.scenes)) === JSON.stringify(expectedScenes), '场景顺序不一致');
  assert(out.characters[firstCharId].displayName === main.characters[firstCharId].displayName, '角色数据丢失');
