...
资源路径约定：背景 `res://assets/backgrounds/{asset}.png`；立绘 `res://assets/characters/{character}/{emotion}.png`；CG `res://assets/cg/{id}.png`；BGM `res://assets/bgm/{track}.(ogg|mp3|wav)`（按此顺序探测）；音效 `res://assets/se/{sound}.wav`。新指令应在 `CommandRunner.Execute` 的 switch 中注册，并同步在 `tools/story-editor/editor.js` 的 `CMD_DEFS` 中登记，保证剧情编辑器能编辑与校验新指令。
